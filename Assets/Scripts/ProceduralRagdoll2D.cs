...
/// <summary>
/// Builds a simple 2D ragdoll out of rectangular sprites, keeps a selectable
/// anchor (head or torso) for downstream scripts, and optionally creates a floor.
/// </summary>
public class ProceduralRagdoll2D : MonoBehaviour
{
    [Header("Visuals")]
    [SerializeField] private Color bodyColor = new(0.95f, 0.95f, 0.95f, 1f);
    [SerializeField] private Color floorColor = new(0.6f, 0.6f, 0.6f, 1f);
    ...
    [Header("Floor")]
    [SerializeField] private Vector2 floorSize = new(12f, 1f);
    [SerializeField] private float floorYOffset = -2.25f;

    [Header("Build Options")]
    [SerializeField] private bool autoBuildOnStart = true;
    [SerializeField] private RagdollAnchor primaryAnchor = RagdollAnchor.UpperTorso;
    [SerializeField] private string centerHandleName = "BodyCenter";

    private readonly Dictionary<string, RagdollPart> _parts = new();
    private Sprite _cubeSprite;
    private Transform _centerHandle;
    private RagdollPart? _primaryPart;

    private static readonly PartDefinition[] BodyDefinitions =
    {
        new("Head", new Vector2(0.9f, 0.9f), new Vector2(0f, 1.9f)),
        new("UpperTorso", new Vector2(0.7f, 0.85f), new Vector2(0f, 1.15f)),
        new("LowerTorso", new Vector2(0.65f, 0.75f), new Vector2(0f, 0.35f)),
        new("LeftUpperArm", new Vector2(0.55f, 0.18f), new Vector2(-0.75f, 1.25f)),
        new("LeftLowerArm", new Vector2(0.45f, 0.17f), new Vector2(-1.05f, 0.8f)),
        new("LeftHand", new Vector2(0.22f, 0.18f), new Vector2(-1.25f, 0.55f)),
        new("RightUpperArm", new Vector2(0.55f, 0.18f), new Vector2(0.75f, 1.25f)),
        new("RightLowerArm", new Vector2(0.45f, 0.17f), new Vector2(1.05f, 0.8f)),
        new("RightHand", new Vector2(0.22f, 0.18f), new Vector2(1.25f, 0.55f)),
        new("LeftUpperLeg", new Vector2(0.4f, 0.75f), new Vector2(-0.25f, -0.35f)),
        new("LeftLowerLeg", new Vector2(0.35f, 0.75f), new Vector2(-0.35f, -1.1f)),
        new("LeftFoot", new Vector2(0.55f, 0.2f), new Vector2(-0.35f, -1.55f)),
        new("RightUpperLeg", new Vector2(0.4f, 0.75f), new Vector2(0.25f, -0.35f)),
        new("RightLowerLeg", new Vector2(0.35f, 0.75f), new Vector2(0.35f, -1.1f)),
        new("RightFoot", new Vector2(0.55f, 0.2f), new Vector2(0.35f, -1.55f))
    };
    ...
    private void BuildRagdoll()
    {
        ...
        Connect("LeftUpperArm", Edge.Right, "UpperTorso", Edge.Left, -70f, 50f);
        Connect("LeftLowerArm", Edge.Right, "LeftUpperArm", Edge.Left, -70f, 30f);
        Connect("LeftHand", Edge.Right, "LeftLowerArm", Edge.Left, -40f, 40f);

        Connect("RightUpperArm", Edge.Left, "UpperTorso", Edge.Right, -70f, 50f);
        ...
        Connect("LeftUpperLeg", Edge.Top, "LowerTorso", Edge.Bottom, -20f, 25f);
        Connect("LeftLowerLeg", Edge.Top, "LeftUpperLeg", Edge.Bottom, -5f, 90f);
        Connect("LeftFoot", Edge.Top, "LeftLowerLeg", Edge.Bottom, -20f, 25f);

        Connect("RightUpperLeg", Edge.Top, "LowerTorso", Edge.Bottom, -20f, 25f);
        Connect("RightLowerLeg", Edge.Top, "RightUpperLeg", Edge.Bottom, -5f, 90f);
        Connect("RightFoot", Edge.Top, "RightLowerLeg", Edge.Bottom, -20f, 25f);
    }
    ...
    private void HydrateExistingParts()
    {
        ...
        _centerHandle = LocateCenterHandle();
    }
    ...
    public Transform CenterTransform => _centerHandle != null
        ? _centerHandle
        : _primaryPart.HasValue ? _primaryPart.Value.GameObject.transform : null;

    public Rigidbody2D PrimaryRigidbody => _primaryPart.HasValue ? _primaryPart.Value.Rigidbody : null;

    private void ResolvePrimaryPart()
    {
        if (_parts.Count == 0)
        {
            _primaryPart = null;
        ...
        var desiredName = primaryAnchor == RagdollAnchor.Head ? "Head" : "UpperTorso";
        if (_parts.TryGetValue(desiredName, out var part))
        {
            _primaryPart = part;
            EnsureCenterHandle(part);
            return;
        }

        if (_centerHandle != null && _centerHandle.parent != null &&
            _parts.TryGetValue(_centerHandle.parent.name, out var handleParent))
        {
            _primaryPart = handleParent;
            EnsureCenterHandle(handleParent);
            return;
        }

        foreach (var entry in _parts.Values)
        {
            _primaryPart = entry;
            EnsureCenterHandle(entry);
            return;
        }
    ...
    private void EnsureCenterHandle(RagdollPart part)
    {
        ...
        if (_centerHandle == null)
        {
            _centerHandle = new GameObject(centerHandleName).transform;
        }

        _centerHandle.SetParent(part.GameObject.transform, false);
        _centerHandle.localPosition = Vector3.zero;
        _centerHandle.localRotation = Quaternion.identity;
        _centerHandle.localScale = Vector3.one;
    }

    private Transform LocateCenterHandle()
    {
